...
TEST_CASE("Testing Database insert_key") {
    ...
    const std::map<std::string, std::shared_ptr<Object>>& data = database->get_data();
...
TEST_CASE("Testing multiple transactions") {
    ...
    CHECK(global_database->select_latest("radu", Object::get_current_time()).value().get()->asString() == "40");
    CHECK(global_database->select_latest("razvan", Object::get_current_time()).value().get()->asString() == "40");
}
TEST_CASE("Testing phantom reads inside of transactions") {
    ...
    CHECK(transaction_2->get_local_store()->select("radu").value().get()->asString() == "30");
    transaction_2->handle_command("EXEC");
}
